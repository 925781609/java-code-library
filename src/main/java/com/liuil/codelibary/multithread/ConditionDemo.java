...
public class ConditionDemo {
  ...
  public static void main(String[] args) {
    ...
    new SingalThread("thread4", lock, condition).start();
  }
  ...
  static class AwaitThread extends Thread {
    ...
    public void run() {
      lock.lock();
      try {
        System.out.println(Thread.currentThread().getName() + " Stait await");
  ...
  static class SingalThread extends Thread {

    private Lock lock;
    private Condition condition;

    public SingalThread(String name, Lock lock, Condition condition) {
      super(name);
      this.lock = lock;
      this.condition = condition;
    }
